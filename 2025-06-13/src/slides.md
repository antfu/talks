...
<!--
Vueのスコープ付きCSSについて、原理を少し説明します。

SFCで`<style scoped>`を使うと、スタイルはそのコンポーネントだけに影響します。

本番環境では、Vueは各DOM要素にdata<ruby>属性<rt>ぞくせい</rt></ruby>を<ruby>追加<rt>ついか</rt></ruby>し、生成された CSS に Selector を追加することで、スコープを実現します。
...
<!--
スコープ付きCSSは素晴らしいツールですが、<ruby>制限<rt>せいげん</rt></ruby>があります。

data 属性を注入することで、スコープ付きCSS は 内部のスタイルが外に<ruby>漏<rt>も</rt></ruby>れるのを<ruby>防<rt>ふせ</rt></ruby>ぐことができます。しかし、考えてみると、[click]内部のスタイルが外に漏れるのを防ぐだけで、[click]外部のスタイルが中に入ってくるのを防ぐことはできません。

[click]つまり、スコープ付きCSS はアプリ全体を<ruby>制御<rt>せいぎょ</rt></ruby>できる場合のは素晴らしいツールです。

[click]しかし、これは<ruby>埋め込み<rt>うめこみ</rt></ruby>アプリのケースでは十分ではありません。ユーザーのスタイルを<ruby>制御<rt>せいぎょ</rt></ruby>できないからです。
...
<!--
え？なんで WebComponents？と思う かもしれません。
-->
...
<img v-click="2" src="/wc-lea.png" alt="The failed promise of Web Components" op75 border="~ main rounded-lg" shadow-2xl w-100 absolute left-8 top-68 />
<img v-click="2" src="/wc-mayank.png" alt="WebComponents considered harmful" border="~ main rounded-lg" shadow-2xl w-120 absolute left-100 top-98 />
<img v-click="1" src="/wc-ryan.png" alt="WebComponents Are Not the Future" border="~ main rounded-lg" shadow-2xl w-150 absolute left-90 top-45 />

<!--
WebComponents について、実は、有名な開発者たちが書いた こういう投稿を見たことがあるかもしれません。

例えば、Svelte の作者の Rich Harris さんは、なぜ WebComponents を使わないか説明しています。

[click] また、Solid の作者の Ryan Carniato さんは、"WebComponents は未来ではない" と言っています。

[click] そのほかにも、たくさんの開発者が WebComponents に対して否定的な意見を<ruby>述<rt>の</rt></ruby>べています。
...
その中では複雑な DOM tree を<ruby>扱<rt>あつか</rt></ruby>えますが、Main Document からは分かれています。
...
<!--
WebComponents を使ったことがある人は、その複雑さを知っているはずです。データの<ruby>更新<rt>こうしん</rt></ruby>を自分で管理しなければならず、[click]CSSを文字列として書く必要があり、Syntax highlighting や Autocomplete も使えません。

スタイルの隔離だけのために、Vueからこんなものに移行するのは、あまり価値がないように感じます。

[click] それは面倒くさいじゃない？

[click] だからこそ、両方の世界の最高の部分を手に入れたいのです。使い慣れたツールを使い続け、開発者体験も維持したい。Vueでロジックを、UnoCSSでスタイリングを実現したい。左側ではなく右側のようなものを望んでいます。

では、そのようなワークフローの設定方法を説明します。
-->
...
<!--
UnoCSSは使った<ruby>分<rt>ふん</rt></ruby>だけ on-demand でCSSを生成するから、ファイルを読み込んでUnoCSSに手動で渡す必要があります。
...
<!--
今日話した ことについて、 starter template を作りました。
